...
<body>
	...
	<div class="page-container">
		<span id="toolbar" class="l"> <a href="javascript:;"
			onclick="datadel()" class="btn btn-danger radius"><i
				class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;"
			onclick="permission_add('添加权限','permission/edit.do','800','500')"
...
// 		菜单格式化
		function menuTypeFormatter(value, row, index) {
			var html = '<span style="color:blue">普通权限</span>';
			if(value=="menu"){
		...
		function operationFormatter(value, row, index) {
			var html = '<a title="编辑" href="javascript:;" onclick="permission_edit('
					+ value
					+ ')" style="text-decoration:none"><i class="Hui-iconfont"></i></a>';
			html += '<a title="删除" href="javascript:;" onclick="permission_del('
					+ value
		...
		function permission_edit(permissionId) {
			layer_show('添加权限',
					'permission/edit.do' + '?permissionId=' + permissionId, '800',
					'500');
		}
		/*权限-删除*/

		function permission_del(data) {
			layer.confirm('确认要删除吗？', {
				icon : 3
			}, function(index) {
				$.ajax({
					type : 'GET',
					url : 'permission/delect.do',
					data : {
						delList : data
					},
		...
		function datadel() {
			...
			checkList.forEach(function(element, index) {
				delList.push(element.permissionId);
			})
			permission_del(delList)
		}
	</script>
</body>
</html>